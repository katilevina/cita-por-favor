...
![Меню Tampermonkey после включения: всё готово](img/setup-07-menu-ready.png)
...
* **«Офисы, которые проверяем»** — галочки: ничего не отмечено = вся
  провинция (запишетесь в любой офис со свободным слотом); один офис =
  проверка только его; два и больше = каждый заход проверяет всю
  провинцию, но записывает только в отмеченные офисы, чужие не берёт.
  Галка «Если выбранных офисов нет — брать любой» (видна при двух и более
  отмеченных) разрешает записаться в любой офис.
...
Файл `Запустить-уведомления-сит.command` скачайте из
[репозитория](https://github.com/katilevina/cita-por-favor/blob/main/Запустить-уведомления-сит.command)
кнопкой **«Download raw file»** — это единственный файл, который нужно
скачать: userscript копируется, а этот запускается двойным кликом.

Запустите скачанный файл двойным кликом — и когда ситы найдутся, компьютер
сам подзовёт вас голосом и системным уведомлением. Можно
заниматься своими делами и не смотреть на вкладку: панель продолжит
проверку, а голос позовёт именно в нужный момент.

Если при первом запуске macOS предупредит, что файл скачан из интернета,
нажмите на него правой кнопкой → **«Открыть»** → ещё раз **«Открыть»**, либо
«Системные настройки» → «Конфиденциальность и безопасность» → **«Все равно
открыть»**. Это стандартная проверка Gatekeeper для скачанных файлов,
предупреждение появится только один раз.
